...
// Hàm Quét qua toàn bộ source code và trả ra các token trong đó
pub fn scanTokens(source: []const u8) !ArrayList(Token) {
    var tokens: ArrayList(Token) = ArrayList(Token).init(std.heap.page_allocator);
        ...
        switch (source[current_i]) {
            ...
            // Bắt đầu xét các trường hợp như từ khoá, literal hoặc các trường hợp khác
                ...
                if (std.ascii.isAlphabetic(source[current_i])) {
                    ...
                    var next_i: usize = current_i + 1; // Bắt đầu kiểm tra từ vị trí current_i + 1
                    ...
                    // Lấy ra string từ vị trí current_i đến next_i
                    ...
                    // Kiểm tra xem token có phải là char literal hay không
                    var next_i: usize = current_i + 1;
                    while (source[next_i] != '\'' and next_i < source.len - 1) {
                        // Nếu next_i đã vượt quá độ dài còn lại của source mà vẫn không tìm thấy ký tự `'` còn lại thì thông báo lỗi và kết thúc hàm
                        if (next_i == source.len) {
                            try reportLexecalError(LexecalError.IlligalCharacterLiteral, current_line);
                            return LexecalError.IlligalCharacterLiteral;
                        }
                        next_i += 1;
                    }
                    const literal: []const u8 = source[current_i .. next_i + 1];
                    // Thực hiện bước kiểm tra này đồng nghĩa với việc đã tìm thấy ký tự `'` còn lại
                    if (literal.len == 3) { // Xác nhận độ dài của char literal hợp lệ
                        const token: Token = Token.init(TokenType.CHAR_LITERAL, literal, current_line);
                        try tokens.append(token);
                        added_token_length = token.lexeme.len;
                    } else {
                        try reportLexecalError(LexecalError.IlligalCharacterLiteral, current_line);
                    ...
                    try reportLexecalError(LexecalError.UndefinedToken, current_line);
...
fn reportLexecalError(err: LexecalError, line: u128) !void {
    try stdout.print("Errors happend `{!}` at line: {}\n", .{ err, line });
}
